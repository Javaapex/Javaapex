...
class MicroserviceReadinessAnalyzer:
    def __init__(self) -> None:
        ...
        self.max_file_inspections = max(24, int(os.getenv("MICROSERVICE_ELIGIBILITY_MAX_FILE_INSPECTIONS", "160")))
        ...
        for current_root, dir_names, file_names in os.walk(root):
            ...
            for file_name in file_names:
                if not file_name.endswith(".java"):
                    continue
                java_file_paths.append(Path(current_root) / file_name)

        java_file_paths.sort(key=self._java_scan_priority)
        if len(java_file_paths) > self.max_file_inspections:
            logger.info(
                "Microservice readiness scan truncated total_java_files=%s max_file_inspections=%s root=%s",
                len(java_file_paths),
                self.max_file_inspections,
                root,
            )

        for file_path in java_file_paths[: self.max_file_inspections]:
            try:
    ...
    def _java_scan_priority(self, file_path: Path) -> tuple[int, int, str]:
        normalized_path = file_path.as_posix().lower()
        priority = 3

        if "/src/main/java/" in normalized_path:
            priority = 0
        elif "/src/main/" in normalized_path:
            priority = 1
        elif "/src/test/java/" in normalized_path or normalized_path.endswith(("test.java", "tests.java")):
            priority = 4
        elif "/test/" in normalized_path:
            priority = 5
    ...
    def _dominant_base_prefix(self, packages: List[str]) -> List[str]:
        common_prefix = common_package_prefix(packages)
