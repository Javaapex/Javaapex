...
class MigrationRequest(BaseModel):
    ...
    llm_test_provider: str = Field(default="ollama", description="LLM provider (openai | groq | deepseek | huggingface | ollama | offline; fordllm aliases to openai)")
    ...
    fix_business_logic: bool = Field(default=False, description="Attempt to fix business logic issues")
...
async def convert_to_microservices(request: MicroserviceConvertRequest):
    ...
    try:
        ...
        response = {
            ...
            "render_deployment": result.render_deployment,
...
async def preview_migration_changes(request: MigrationRequest):
    ...
    try:
        ...
        # Repository creation and deployment require a GitHub token in this flow.
        if request.platform == GitPlatform.GITHUB and not source_token:
            raise RuntimeError(
                "GitHub token is required for generation because output must be pushed to a new repository "
                "before deployment. Provide a PAT with repo or public_repo scope."
            )
...
async def run_migration(job_id: str, request: MigrationRequest):
  ...
  # Step 4: Run tests
        if request.run_tests:
            ...
            llm_provider = request.llm_test_provider or "ollama"
        ...
        if request.migration_type == "microservices":
            ...
            try:
                ...
                job.render_deployment = conversion_result.render_deployment
                if conversion_result.render_deployment:
                    add_log(
                        job_id,
                        f"Microservice deployment handoff status: {conversion_result.render_deployment.get('status', 'unknown')}",
                    )
        ...
        push_status_message = "Creating new repository and pushing migrated code..."
        ...
        try:
            ...
            raise RuntimeError(f"Failed to create/push new repository for generation: {push_error}") from push_error

        # Step 6b: Deploy from generated repository.
        # Enforce GitHub Actions deployment path by default.
        # Backend Render API deployment is allowed only with an explicit opt-in gate.
        ...
        direct_render_api_mode = deploy_mode in ("render_api", "render-api", "render") and allow_backend_render
        if not direct_render_api_mode:
            render_result = {
            ...
            add_log(job_id, "Render deployment mode: github_actions")
            add_log(job_id, "GitHub Actions workflow was committed. Render deploy will be triggered by repository push.")
        else:
            add_log(job_id, "Backend Render API deploy is explicitly enabled via ALLOW_BACKEND_RENDER_DEPLOY.")
            default_service_name = f"{source_repo_name}-java{request.target_java_version.value}"
            render_result = render_deployment_service.deploy_repository(
                repo_reference=job.target_repo or "",
                service_name=default_service_name,
            )
            job.render_deployment = render_result
            add_log(job_id, f"Render deployment status: {render_result.get('status', 'unknown')}")
            if render_result.get("reason"):
                add_log(job_id, f"Render deployment reason: {render_result.get('reason')}")
            if render_result.get("accessible_url"):
                add_log(job_id, f"Render deployed URL: {render_result.get('accessible_url')}")
            elif render_result.get("dashboard_url"):
                add_log(job_id, f"Render dashboard: {render_result.get('dashboard_url')}")
