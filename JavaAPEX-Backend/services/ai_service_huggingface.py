"""
AI Service with OpenAI-Compatible Integration
Provides advanced code analysis using OpenAI-Compatible API (OpenAI-compatible)
"""
import os
import json
import logging
...
# Model mapping – all analysis types route through OpenAI-Compatible
MODELS = {
...
class HuggingFaceAIService:
    """AI Service using OpenAI-Compatible API for code analysis (kept class name for backward compatibility)"""
    ...
    def __init__(self):
        ...
        logger.info(f"OpenAI-Compatible AI Service initialized (available: {self.available})")
    ...
    async def query_huggingface(self, model_id: str, prompt: str, max_length: int = 1024) -> str:
        """Query OpenAI-Compatible API (method name kept for backward compatibility)"""
        if not self.available:
            return "OpenAI-Compatible credentials not configured"
        ...
        try:
            ...
            completion = await loop.run_in_executor(
                ...
                lambda: client.chat.completions.create(
                    ...
                    temperature=0.3,
                    top_p=0.9,
                    **({"extra_body": {"models": [self.sub_model]}} if self.sub_model else {}),
                ),
            )
            ...
            if "401" in str(e) or "unauthorized" in str(e).lower():
                logger.warning("OpenAI-Compatible 401 – refreshing token and retrying …")
                try:
                    ...
                    logger.error("OpenAI-Compatible retry failed: %s", retry_exc)
                    return f"Error: {str(retry_exc)}"
            logger.error(f"Error querying OpenAI-Compatible: {e}")
