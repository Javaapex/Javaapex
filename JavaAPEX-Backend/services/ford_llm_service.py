...
class FordLLMService:
        ...
        *,
        ...
        for idx, try_model in enumerate(models_to_try):
            try:
                ...
                # Non-empty HTTP 200 but empty content — try next model
                logger.warning(
    ...
    def _extract_text(self, data: Dict[str, Any]) -> str:
        ...
        Handles standard ``choices[0].message.content`` as well as alternate
        layouts used by reasoning / newer models (``output``, ``reasoning_content``,
        ``parts``, ``refusal``).  Returns empty string instead of raising so the
        model-fallback loop can try the next model.
        """
        # --- Standard OpenAI choices layout ---
        ...
        if isinstance(choices, list) and choices:
            ...
            if isinstance(message, dict):
                ...
                # Some models nest output in message.parts
                ...
                # Refusal field (model declined but gave a reason)
                refusal = message.get("refusal")
                if isinstance(refusal, str) and refusal.strip():
                    logger.warning("Model refused request: %s", refusal[:200])
                    return ""  # treat refusal as empty so fallback kicks in

            # delta field (streaming leftovers)
        ...
        # Some endpoints return {"output": "..."} directly
        ...
        # Nothing usable — return empty so the model-fallback loop can advance
