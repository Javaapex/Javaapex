...
class GitHubCloneAnalysisService:
    ...
    async def analyze_repository(
        ...
        if workspace.snapshot_id and not analysis.get("repo_snapshot_id"):
            analysis["repo_snapshot_id"] = workspace.snapshot_id
        if workspace.revision and not analysis.get("workspace_revision"):
            analysis["workspace_revision"] = workspace.revision
    ...
    async def stage_workspace_copy(
        ...
        folder_name = f"{name_prefix}-{uuid.uuid4().hex}" if name_prefix else uuid.uuid4().hex
        working_copy_path = os.path.join(target_root, folder_name)
        await self.workspace_service.stage_workspace_copy(
            repo_reference=repo_reference,
            destination_path=working_copy_path,
            token=token,
            force_refresh=force_refresh,
        )
