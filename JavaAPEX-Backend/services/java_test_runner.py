...
async def _run_once(
    ...
    try:
        ...
        try:
            ...
            logger.warning("asyncio subprocess is not supported in this runtime; falling back to synchronous process execution.")
            fallback_cmd = list(cmd)
            if os.name == "nt" and fallback_cmd and fallback_cmd[0].lower() == "mvn":
                fallback_cmd[0] = "mvn.cmd"
            ...
            raw_output = ((completed.stdout or b"").decode(errors="replace") + (completed.stderr or b"").decode(errors="replace")).strip()
