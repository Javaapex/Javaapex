...
class LocalRepositoryAnalysisService:
    ...
    def _analyze_workspace_sync(self, workspace: RepositoryWorkspace) -> Dict[str, Any]:
        ...
        android_info = self._detect_android_project(root, gradle_files)
        ...
        structure = {
            ...
            "has_build_gradle_kts": any(path.name == "build.gradle.kts" for path in gradle_files),
        }
        ...
        analysis: Dict[str, Any] = {
            ...
            "build_files_info": {
                "pom_files": [self._build_file_descriptor(root, file_path) for file_path in pom_files],
                "gradle_files": [self._build_file_descriptor(root, file_path) for file_path in gradle_files],
    ...
    def _detect_android_project(self, root: Path, gradle_files: Iterable[Path]) -> Dict[str, Any]:
        """
        Best-effort Android detection so the UI/pipeline can pick the right test strategy.

        Returns:
          {
            "is_android": bool,
            "modules": [ "app", "feature/foo", ... ],
            "uses_ndk": bool
          }
        """
        is_android = False
        ...
        for gradle_file in gradle_files:
            ...
            if "com.android.application" in text or "com.android.library" in text:
                is_android = True
                try:
                    rel = gradle_file.parent.relative_to(root)
                    modules.append(str(rel).replace("\\", "/") or ".")
                except Exception:
                    modules.append(str(gradle_file.parent).replace("\\", "/"))

            if "externalnativebuild" in text or "ndkversion" in text or "cmake" in text or "ndk {" in text:
                uses_ndk = True

        # Also detect NDK usage via common native build files.
        if not uses_ndk:
            for name in ("CMakeLists.txt", "Android.mk", "Application.mk"):
                try:
        ...
        return {"is_android": bool(is_android), "modules": uniq, "uses_ndk": bool(uses_ndk)}
    ...
    def _collect_all_files(self, root: Path) -> List[Dict[str, Any]]:
        ...
        for current_root, dir_names, file_names in os.walk(root):
            ...
            for file_name in sorted(file_names):
                ...
                try:
                    ...
                    logger.warning("Skipping file outside workspace boundary during local analysis: %s", file_path)
                    continue
                try:
    ...
    def _build_file_descriptor(self, root: Path, file_path: Path) -> Dict[str, Any]:
        relative_path = self._relative_path(root, file_path)
        ...
        try:
            descriptor["size"] = file_path.stat().st_size
        except OSError:
            descriptor["size"] = 0
        return descriptor
        ...
        for pom_file in pom_files:
            ...
            for match in pattern.finditer(content):
                ...
                if dependency in seen:
                    continue
                seen.add(dependency)
                guidance = self.maven_central_service.get_upgrade_guidance(
                    dependency[0],
                    dependency[1],
                    dependency[2],
                )
                ...
                dependencies.append(
                    {
                        ...
                        "new_version": guidance.get("new_version"),
                        "status": status,
                        "version_source": guidance.get("source"),
                    }
                )
