...
class MigrationService:
    ...
    async def run_migration(
        ...
        if not os.path.exists(pom_path) and not os.path.exists(gradle_path):
            ...
            print(f"âœ“ Converted standalone project to Maven structure")
        ...
        # Apply migrations to all Java directories (deduplicated across overlapping roots)
        processed_files: set = set()
        for src_dir in java_dirs:
            if os.path.exists(src_dir):
                modifications = await self._apply_java_migrations(
                    src_dir,
                    source_version,
                    target_version,
        ...
        # Fix business logic only when both request flag and env gate are enabled.
        env_autofix = os.getenv("ENABLE_BUSINESS_LOGIC_AUTOFIX", "0").strip().lower() in {"1", "true", "yes", "on"}
        if fix_business_logic and env_autofix:
...
"""
            ...
            if content != original_content:
                with open(pom_path, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"âœ“ Updated pom.xml with Java {target_version}")
                return True
    ...
    async def _migrate_java_file(
        ...
        try:
                    ...
                    for i, line in enumerate(content.split('\n')):
                        if old in line or (new in line and '// Migration:' in line):
                            ...
                            highlighted_changes.append({
                                "line_number": i + 1,
                                "before": line.replace(f'{new} // Migration: {desc} - Source: Java {source_version} â†’ Target: Java {target_version}', new),
                                "after": line,
            ...
            if os.path.basename(filepath).lower().endswith('readme.md') and f'// Migrated to Java {target_version}' not in content:
            ...
            for file in files:
                if file.endswith('.java'):
                    ...
                    try:
                        ...
                        # (Skipped â€“ regex-based replacements on raw patterns
                        ...
                        if content != original_content:
                            with open(filepath, 'w', encoding='utf-8') as f:
                                f.write(content)
                            fixes += file_fixes
                            print(f"âœ“ Applied {file_fixes} business logic fixes to {file}")
    ...
    async def run_tests(
        ...
        if use_llm_tests:
            pipeline_data = await llm_test_pipeline.run_pipeline(project_path, llm_provider)
        ...
        try:
            ...
            result["test_insights"] = summary_payload.get("insights", [])
    ...
    async def _run_standard_tests(self, project_path: str) -> Dict[str, Any]:
        ...
        runner = await run_java_tests(project_path, timeout_sec=timeout)
...
"""
    ...
    async def convert_build_file_with_llm(self, build_content: str, conversion_type: str, detected_build_tool: Optional[str] = None) -> str:
        ...
        fordllm_base_url = os.getenv("LLM_BASE_URL", os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1"))
        fordllm_model = os.getenv("LLM_MODEL", os.getenv("OPENAI_MODEL", "gpt-4.1-mini"))
        fordllm_sub_model = os.getenv("LLM_SUB_MODEL", "")

        normalized_build_tool = (detected_build_tool or "").strip().lower() or None
        if normalized_build_tool == "maven" and conversion_type != "maven_to_gradle":
            raise ValueError("Detected Maven build tool, but conversion_type was not maven_to_gradle")
        if normalized_build_tool == "gradle" and conversion_type != "gradle_to_maven":
            raise ValueError("Detected Gradle build tool, but conversion_type was not gradle_to_maven")

        build_context = ""
        if normalized_build_tool:
            build_context = f"Detected source build tool: {normalized_build_tool}. Keep the conversion direction consistent with that source build system.\n"

        # Prompt selection
        """
        if conversion_type == "maven_to_gradle":
            prompt = (
                "You are an expert Java build engineer. Translate the following Maven pom.xml file "
                "into a valid Gradle build.gradle file using Groovy DSL. Map all scopes correctly "
                "(compile -> implementation, test -> testImplementation). Output ONLY the raw Gradle code:\n\n"
                f"{build_content}"
            )
    
        elif conversion_type == "gradle_to_maven":
            prompt = (
                "You are an expert Java build engineer. Translate the following Gradle build.gradle file "
                "into a valid Maven pom.xml file. Map dependencies and configurations properly. "
                "Output ONLY the raw XML code:\n\n"
                f"{build_content}"
            )
        ...
        if conversion_type == "maven_to_gradle":
            prompt = (
                "You are an expert Java build engineer. Translate the following Maven pom.xml file "
                "into a valid Gradle build.gradle file using Groovy DSL. "
                f"{build_context}"
                "CRITICAL REQUIREMENTS FOR BUSINESS LOGIC PRESERVATION:\n"
                "1. PRESERVE ALL DEPENDENCIES: Every <dependency> in pom.xml MUST be converted to Gradle with correct scope mapping\n"
                "2. JAVA PLUGIN: You MUST include `plugins { id 'java' }` at the very top\n"
                "3. JAVA VERSION: Set sourceCompatibility and targetCompatibility to match pom.xml properties (default to Java 17 if not specified)\n"
                "4. SCOPE MAPPING: compile â†’ implementation, test â†’ testImplementation, provided â†’ compileOnly, runtime â†’ runtimeOnly\n"
                "5. PROPERTIES: Convert <properties> block values to Gradle equivalents\n"
                "6. PLUGINS: Convert Maven plugins to equivalent Gradle plugins where possible\n"
                "7. REPOSITORIES: Include Maven Central and any custom repositories from pom.xml\n"
                "8. GROUP/ARTIFACT/VERSION: Preserve project coordinates exactly\n"
                "9. VALID SYNTAX: Output MUST be valid Gradle Groovy DSL that can compile\n"
                "10. NO BUSINESS LOGIC CHANGES: Only convert build configuration, do not modify dependency versions unless upgrading is explicitly required\n\n"
                "Output ONLY the raw Gradle code with no markdown formatting:\n\n"
                f"{build_content}"
            )
            ...
            prompt = (
                "You are an expert Java build engineer. Translate the following Gradle build.gradle file "
                "into a valid, modern Maven pom.xml file. "
                f"{build_context}"
                "CRITICAL REQUIREMENTS FOR BUSINESS LOGIC PRESERVATION:\n"
                "1. PRESERVE ALL DEPENDENCIES: Every dependency in build.gradle MUST be converted to Maven with correct scope mapping\n"
                "2. XML VALIDITY: Output MUST be well-formed XML with proper closing tags\n"
                "3. PROPERTIES: Include <maven.compiler.source> and <maven.compiler.target> matching Gradle's sourceCompatibility/targetCompatibility\n"
                "4. SCOPE MAPPING: implementation â†’ compile, testImplementation â†’ test, compileOnly â†’ provided, runtimeOnly â†’ runtime\n"
                "5. GROUP/ARTIFACT/VERSION: Preserve project coordinates exactly from group/artifact/version\n"
                "6. REPOSITORIES: Convert Gradle repositories to <repositories> block\n"
                "7. PLUGINS: Convert Gradle plugins to equivalent Maven plugins in <build><plugins>\n"
                "8. DEPENDENCY MANAGEMENT: Preserve version management and exclusions\n"
                "9. VALID POM: Output MUST be a valid pom.xml that Maven can read\n"
                "10. NO BUSINESS LOGIC CHANGES: Only convert build configuration, preserve all dependency coordinates\n\n"
                "Output ONLY the raw XML code with no markdown formatting:\n\n"
                f"{build_content}"
            )
                ...
                prompt = (
            "You are an expert Java build engineer and Spring Boot migration specialist. "
            "Convert the following Gradle build.gradle file into ONE valid and working Maven pom.xml file. "
            "This project should end as a Maven project, not a mixed Gradle/Maven project.\n\n"
            ...
            "1. OUTPUT ONLY ONE FILE: Return only the contents of pom.xml as raw XML.\n"
            "2. NO MARKDOWN: Do not use code fences, explanations, headings, or extra text.\n"
            "3. VALID POM: Output a complete, well-formed, modern Maven pom.xml that can build successfully.\n"
            "4. JAVA VERSION: Configure Java 17 using <properties> with <java.version>, <maven.compiler.source>, and <maven.compiler.target>.\n"
            "5. DEPENDENCY PRESERVATION: Preserve all Gradle dependencies unless a compatibility fix is required for Java 17 / Spring Boot 3.\n"
            "6. SCOPE MAPPING:\n"
            "   - implementation -> compile\n"
            "   - testImplementation -> test\n"
            "   - compileOnly -> provided\n"
            "   - runtimeOnly -> runtime\n"
            "   - annotationProcessor -> maven-compiler-plugin annotationProcessorPaths when needed\n"
            "7. VALID SPRING BOOT DEPENDENCIES:\n"
            "   - use valid Spring Boot starter dependencies\n"
            "   - do not generate invalid, duplicate, or contradictory dependencies\n"
            "8. VERSION MANAGEMENT:\n"
            "   - use Spring Boot dependency management or spring-boot-starter-parent when appropriate\n"
            "   - do not pin versions for Spring Boot starter dependencies unless absolutely necessary\n"
            "   - preserve explicit versions for non-Boot libraries where needed\n"
            "9. LOMBOK: If Lombok exists in build.gradle, configure it correctly for Maven compilation.\n"
            "10. REPOSITORIES: Include Maven Central and any custom repositories from build.gradle.\n"
            "11. PROJECT COORDINATES: Preserve group, artifactId, and version exactly.\n"
            "12. PROPERTIES: Convert Gradle ext/properties into Maven properties where needed.\n"
            "13. PLUGINS: Convert Gradle plugin behavior to equivalent Maven plugins where possible.\n"
            "14. JAKARTA COMPATIBILITY: Assume Spring Boot 3 compatibility. Preserve jakarta.* usage and do not reintroduce javax.* except JDK-owned packages if necessary.\n"
            "15. NO GRADLE LEFTOVERS: Do not reference gradlew, gradlew.bat, gradle wrapper files, settings.gradle, or Gradle-only commands in the output.\n"
            "16. NO MIXED BUILD TOOLS: The generated pom.xml must be fully Maven-native and must not depend on any Gradle wrapper or Gradle directory.\n"
            "17. SPRING BOOT BUILD SUPPORT: If this is a Spring Boot project, include the correct Spring Boot Maven plugin configuration.\n"
            "18. PRESERVE BUSINESS LOGIC: Only convert build configuration; do not change application logic.\n"
            "19. Generate a complete, runnable Maven project definition so the project can be built immediately with mvn clean install with minimal or no manual changes.\n\n"

            "SUCCESS CONDITION:\n"
            "The output pom.xml must be suitable for a Maven project where Gradle wrapper leftovers can be removed separately, "
            "and it should support a normal Maven build and test run immediately.\n\n"
        ...
        if conversion_type == "maven_to_gradle":
            ...
            "This project should end as a Gradle project, not a mixed Maven/Gradle project.\n\n"
            ...
            "CRITICAL REQUIREMENTS:\n"
            "1. OUTPUT ONLY ONE FILE: Return only the contents of build.gradle as raw Gradle Groovy DSL.\n"
            "2. NO MARKDOWN: Do not use code fences, explanations, headings, or extra text.\n"
            "3. JAVA PLUGIN: Include the correct plugins for a Spring Boot Gradle project:\n"
            "   - id 'java'\n"
            "   - id 'org.springframework.boot' version '3.2.5'\n"
            "   - id 'io.spring.dependency-management' version '1.1.4'\n"
            "4. JAVA VERSION: Configure Java 17 using the Gradle java { } block or toolchain. "
            "Do NOT use loose root-level sourceCompatibility or targetCompatibility assignments.\n"
            "5. DEPENDENCY PRESERVATION: Preserve all Maven dependencies unless a compatibility fix is required for Java 17 / Spring Boot 3.\n"
            "6. SCOPE MAPPING:\n"
            ...
            "   - runtime -> runtimeOnly\n"
            "7. VALID SPRING BOOT DEPENDENCIES:\n"
            "   - use valid starters such as spring-boot-starter-web and spring-boot-starter-test\n"
            "   - do not generate invalid starters such as spring-boot-starter-webmvc-test\n"
            "8. VERSION MANAGEMENT:\n"
            "   - do not pin versions for Spring Boot starter dependencies unless absolutely necessary\n"
            "   - preserve explicit versions for non-Boot libraries where needed\n"
            "9. LOMBOK: If Lombok exists in pom.xml, include compileOnly and annotationProcessor entries.\n"
            "10. REPOSITORIES: Include Maven Central and any custom repositories from pom.xml.\n"
            "11. PROJECT COORDINATES: Preserve group and version exactly.\n"
            "12. PROPERTIES: Convert Maven properties into Gradle equivalents where needed.\n"
            "13. PLUGINS: Convert Maven plugin behavior to Gradle where possible.\n"
            "14. JAKARTA COMPATIBILITY: Assume Spring Boot 3 compatibility. Preserve jakarta.* usage and do not reintroduce javax.* except JDK-owned packages if necessary.\n"
            "15. NO MAVEN LEFTOVERS: Do not reference Maven wrapper files, mvnw, mvnw.cmd, .mvn, pom.xml plugins, or any Maven-only commands in the output.\n"
            "16. NO MIXED BUILD TOOLS: The generated build.gradle must be fully Gradle-native and must not depend on any Maven wrapper or Maven directory.\n"
            "17. VALID SYNTAX: Output must be syntactically correct Gradle Groovy DSL.\n"
            "18. PRESERVE BUSINESS LOGIC: Only convert build configuration; do not change application logic.\n\n"
            "19. Generate a complete, runnable Gradle project and include all required build files: build.gradle, settings.gradle, gradlew, gradlew.bat, gradle/wrapper/gradle-wrapper.properties, and gradle/wrapper/gradle-wrapper.jar, so the project can be built and run immediately with ./gradlew build without any manual setup or downloads."
            "SUCCESS CONDITION:\n"
            "The output build.gradle must be suitable for a Gradle project where Maven wrapper leftovers should be removed separately, "
            "and it should support running Gradle wrapper generation and then a normal Gradle build.\n\n"
        ...
        "You are an expert Java build engineer and Spring Boot migration specialist. "
        "Convert the following Gradle build.gradle file into ONE valid and working Maven pom.xml file. "
        "This project must end as a pure Maven project, not a mixed Gradle/Maven project.\n\n"
        ...
        "OUTPUT RULES:\n"
        "1. OUTPUT ONLY ONE FILE: Return only the contents of pom.xml as raw XML.\n"
        "2. NO MARKDOWN: Do not use code fences, explanations, headings, comments, or extra text.\n"
        "3. COMPLETE FILE ONLY: Output a fully complete pom.xml from the XML declaration through the closing </project> tag.\n"
        "4. NO TRUNCATION: Do not stop early, do not omit closing tags, and do not leave partial elements or placeholder tags.\n"
        "5. WELL-FORMED XML: Every opened tag must be properly closed; the final output must be valid parseable XML.\n\n"

        "POM REQUIREMENTS:\n"
        "6. VALID POM: Output a complete, well-formed, modern Maven pom.xml that can build successfully.\n"
        "7. JAVA VERSION: Configure Java 17 using <properties> with <java.version>, <maven.compiler.source>, and <maven.compiler.target>.\n"
        "8. PROJECT COORDINATES: Preserve groupId, artifactId, and version exactly.\n"
        "9. PROPERTIES: Convert Gradle ext/properties into Maven properties where needed.\n"
        "10. REPOSITORIES: Include Maven Central and any custom repositories from build.gradle.\n"
        "11. VERSION MANAGEMENT:\n"
        "    - Prefer spring-boot-starter-parent or Spring Boot dependency management.\n"
        "    - Do not pin versions for Spring Boot starter dependencies unless strictly required.\n"
        "    - Preserve explicit versions for non-Boot libraries where needed.\n"
        "12. DEPENDENCY PRESERVATION: Preserve all Gradle dependencies unless a compatibility fix is required for Java 17 / Spring Boot 3.\n"
        "13. SCOPE MAPPING:\n"
        "    - implementation -> compile\n"
        "    - testImplementation -> test\n"
        "    - compileOnly -> provided\n"
        "    - runtimeOnly -> runtime\n"
        "    - annotationProcessor -> maven-compiler-plugin annotationProcessorPaths when needed\n"
        "14. VALID DEPENDENCIES ONLY:\n"
        "    - Do not invent Maven scopes that do not exist.\n"
        "    - Do not generate unsupported scopes like 'checkstyle'.\n"
        "    - Do not create duplicate or contradictory dependencies.\n"
        "15. JAKARTA COMPATIBILITY: Assume Spring Boot 3 compatibility. Preserve jakarta.* usage and do not reintroduce javax.* except JDK-owned packages if necessary.\n\n"

        "PLUGIN REQUIREMENTS:\n"
        "16. PLUGINS: Convert Gradle plugin behavior to equivalent Maven plugins where possible.\n"
        "17. SPRING BOOT BUILD SUPPORT: If this is a Spring Boot project, include a correct Spring Boot Maven plugin configuration.\n"
        "18. COMPILER PLUGIN: Include a correct maven-compiler-plugin configuration for Java 17.\n"
        "19. PARAMETER METADATA: Enable Spring-friendly parameter metadata by setting <parameters>true</parameters> in maven-compiler-plugin.\n"
        "20. LOMBOK: If Lombok exists in build.gradle, configure it correctly for Maven compilation.\n"
        "21. TEST SUPPORT: Preserve test dependencies and test plugin compatibility needed for Spring Boot tests.\n\n"

        "STRICT SAFETY RULES:\n"
        "22. ONLY BUILD CONVERSION: Convert only build configuration. Do not change application source code, controller signatures, imports, validation logic, packages, or comments in Java files.\n"
        "23. NO BUSINESS LOGIC CHANGES: Do not add null checks, do not alter method behavior, and do not infer code refactors outside pom.xml.\n"
        "24. NO PLACEHOLDERS: Do not output incomplete tags such as <artifactId> with no value.\n"
        "25. NO COMMENTED BROKEN CONTENT: Do not leave commented-out fragments of failed XML or duplicate build sections.\n"
        "26. NO GRADLE LEFTOVERS: Do not reference gradlew, gradlew.bat, Gradle wrapper files, settings.gradle, or Gradle-only commands in the pom.\n"
        "27. NO MIXED BUILD TOOLS: The generated pom.xml must be fully Maven-native and must not depend on any Gradle wrapper or Gradle directory.\n"
        "28. MINIMAL MANUAL FIXES: Generate a complete, runnable Maven project definition so the project can be built immediately with mvn clean install with minimal or no manual changes.\n\n"

        "FINAL SELF-CHECK BEFORE OUTPUT:\n"
        "29. Verify the pom.xml is syntactically complete and ends with </project>.\n"
        "30. Verify all dependencies use valid Maven scopes.\n"
        "31. Verify Spring Boot plugin and compiler plugin are both correctly configured.\n"
        "32. Verify Java 17 and compiler parameter metadata are configured.\n"
        "33. Return only the final raw XML of pom.xml.\n"
    )

        else:
            return build_content

        # FordLLM API call via OpenAI client
        print(f"[LLM] Sending {conversion_type} translation request to FordLLM...")

        try:
            access_token = fordllm_auth.token
            client = OpenAI(api_key=access_token, base_url=fordllm_base_url)
            ...
            completion = await loop.run_in_executor(
                ...
                lambda: client.chat.completions.create(
                    ...
                    max_tokens=1500,
                    temperature=0.1,
                    **({"extra_body": {"models": [fordllm_sub_model]}} if fordllm_sub_model else {}),
                ),
            )
            ...
            if "401" in str(e) or "unauthorized" in str(e).lower():
                print("[LLM] Token expired, refreshing and retrying...")
                access_token = fordllm_auth.refresh_token()
                client = OpenAI(api_key=access_token, base_url=fordllm_base_url)
                loop = asyncio.get_event_loop()
                completion = await loop.run_in_executor(
                    None,
                ...
                print(f"[LLM ERROR] {e}")
                raise Exception(f"FordLLM API failed: {e}")

        return cleaned_text



        # Built conversion code
        """
        async def convert_build_file_with_llm(self, build_content: str, conversion_type: str) -> str:
        Uses Hugging Face Qwen2.5-Coder to convert between Maven and Gradle.
        """
        """
        api_key = os.getenv("HUGGINGFACE_API_KEY")
        if not api_key:
            raise Exception("HUGGINGFACE_API_KEY is not set in your .env file")

        # Pick the best Hugging Face model for code generation
        model_url = "https://api-inference.huggingface.co/models/Qwen/Qwen2.5-Coder-32B-Instruct"
        ...
        # 1. Determine the correct prompt based on conversion type
        if conversion_type == "maven_to_gradle":
            prompt = (
                "You are an expert Java build engineer. Translate the following Maven pom.xml file "
                "into a valid Gradle build.gradle file using Groovy DSL. Map all scopes correctly "
                "(compile -> implementation, test -> testImplementation). Output ONLY the raw Gradle code block:\n\n"
                f"{build_content}"
            )
        elif conversion_type == "gradle_to_maven":
            prompt = (
                "You are an expert Java build engineer. Translate the following Gradle build.gradle file "
                "into a valid Maven pom.xml file. Map dependencies and configurations properly. "
                "Output ONLY the raw XML code block:\n\n"
                f"{build_content}"
            )
        else:
            return build_content # Do nothing if unknown type

        # 2. Build the Hugging Face Serverless API payload
        payload = {
            "inputs": prompt,
            "parameters": {
                "max_new_tokens": 1500, # Build files can be long
        ...
        print(f"[LLM] Sending {conversion_type} translation request to Hugging Face...")
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            # The LLM will usually wrap the code in markdown blocks (```xml or ```gradle). We strip it out:
            cleaned_text = generated_text.replace("```xml", "").replace("```gradle", "").replace("```groovy", "").replace("```", "").strip()
