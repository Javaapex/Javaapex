...
class SonarQubeService:
    ...
    def _build_compile_command(self, project_path: str, build_tool: str) -> List[str]:
        root = Path(project_path)
        maven_repo_local = str((root / ".javaapex-cache" / "m2-repository").resolve())
        if build_tool == "maven":
            if os.name == "nt" and self._has_maven_wrapper(root, "mvnw.cmd"):
                return [
                    str(root / "mvnw.cmd"),
                    "-B",
            ...
            if os.name == "nt" and self._has_maven_wrapper(root, "mvnw.bat"):
                return [
                    str(root / "mvnw.bat"),
                    "-B",
                    f"-Dmaven.repo.local={maven_repo_local}",
            ...
            if os.name != "nt" and self._has_maven_wrapper(root):
                return [
                    "sh",
                    str(root / "mvnw"),
                    "-B",
            ...
            maven_cmd = self._resolve_build_tool_command("maven")
            return [
                maven_cmd,
                "-B",
                f"-Dmaven.repo.local={maven_repo_local}",
        ...
        if build_tool == "gradle":
            if os.name == "nt" and self._has_gradle_wrapper(root, "gradlew.bat"):
                return [str(root / "gradlew.bat"), "--no-daemon", "classes", "-x", "test"]
            if os.name != "nt" and self._has_gradle_wrapper(root, "gradlew"):
                return ["sh", str(root / "gradlew"), "--no-daemon", "classes", "-x", "test"]
            gradle_cmd = self._resolve_build_tool_command("gradle")
            return [gradle_cmd, "--no-daemon", "classes", "-x", "test"]

        raise SonarQubeConfigurationError(f"Unsupported build tool for Sonar analysis: {build_tool}")

    def _has_maven_wrapper(self, root: Path, wrapper_name: str = "mvnw") -> bool:
        wrapper_script = root / wrapper_name
        wrapper_jar = root / ".mvn" / "wrapper" / "maven-wrapper.jar"
        return wrapper_script.exists() and wrapper_jar.exists()

    def _has_gradle_wrapper(self, root: Path, wrapper_name: str = "gradlew") -> bool:
        wrapper_script = root / wrapper_name
        wrapper_jar = root / "gradle" / "wrapper" / "gradle-wrapper.jar"
        return wrapper_script.exists() and wrapper_jar.exists()

    def _resolve_build_tool_command(self, build_tool: str) -> str:
        candidates = self._build_tool_candidates(build_tool)
        ...
        if build_tool == "maven":
            raise SonarQubeConfigurationError(
                "Maven executable was not found. Install Maven on the server or add Maven Wrapper "
                "(mvnw/mvnw.cmd) to the repository so Sonar can compile the project."
            )
        if build_tool == "gradle":
            raise SonarQubeConfigurationError(
                "Gradle executable was not found. Install Gradle on the server or add Gradle Wrapper "
                "(gradlew/gradlew.bat) to the repository so Sonar can compile the project."
            )
        raise SonarQubeConfigurationError(f"Unsupported build tool for Sonar analysis: {build_tool}")

    def _build_tool_candidates(self, build_tool: str) -> List[str]:
        ...
        if build_tool == "maven":
            ...
            candidates.extend(
                self._find_cached_wrapper_binaries(
    ...
    def _find_cached_wrapper_binaries(self, base_dir: str, executable_name: str, limit: int = 12) -> List[str]:
        ...
        try:
            matches = sorted(
                (
                    path for path in root.rglob(executable_name)
                    if path.is_file()
                ),
    ...
    def _resolve_executable_candidate(self, candidate: str) -> Optional[str]:
        if not candidate:
            return None
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            for _ in range(timeout_seconds // interval_seconds):
                ...
                if status in {"FAILED", "CANCELED"}:
                    error_message = str(task.get("errorMessage") or "").strip()
                    if error_message:
                        raise SonarQubeExecutionError(
                            f"Sonar compute-engine task ended with status {status}: {error_message}"
                        )
                    raise SonarQubeExecutionError(
    ...
    async def _run_command(self, command: List[str], cwd: str) -> Dict[str, Any]:
        ...
        command = self._wrap_windows_script(command)
        ...
        try:
            ...
            command_name = command[2] if os.name == "nt" and len(command) >= 3 and str(command[1]).lower() == "/c" else command[0]
            raise SonarQubeExecutionError(f"Failed to start {command_name}: {exc}") from exc
        return {
    ...
    def _wrap_windows_script(self, command: List[str]) -> List[str]:
        if os.name != "nt" or not command:
            return command

        executable = str(command[0] or "").lower()
        if executable.endswith((".bat", ".cmd")):
            comspec = os.environ.get("COMSPEC", "cmd.exe")
            return [comspec, "/c", *command]
        if executable.endswith(".ps1"):
            return ["powershell.exe", "-NonInteractive", "-File", *command]
        return command
    ...
    def _build_command_error(
        ...
        details = self._extract_relevant_command_failure(result)
    ...
    def _extract_relevant_command_failure(self, result: Dict[str, Any]) -> str:
        ...
        for raw_line in combined.splitlines():
            ...
            if "[ERROR]" in upper_line or "ERROR" in upper_line or "EXCEPTION" in upper_line or "FAILED" in upper_line:
    ...
    def _resolve_java_home(self) -> Optional[str]:
        ...
        common_candidates = [
            os.path.join(os.getenv("ProgramFiles", r"C:\Program Files"), "Java", "jdk-21.0.6"),
            os.path.join(os.getenv("ProgramFiles", r"C:\Program Files"), "Java", "latest", "jdk-21"),
